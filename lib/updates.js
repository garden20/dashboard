...
exports.navigation = function(doc, req) {
    ...
    doc.top_nav_bar.hide_home_link = kindyTruthy(req.query.hide_home_link);
...
exports.sessions = function(doc, req) {
    ...
    if (doc.sessions['type'] == 'other') {
        ...
        delete doc.sessions.type_internal;
    }

    if (req.query['require_valid_user'] === 'on')
        doc.sessions.require_valid_user_on = true;
    else
        delete doc.sessions.require_valid_user_on;

    if (req.query['session_persist'] === 'on')
        doc.sessions.session_persist = true;
    else
        delete doc.sessions.session_persist

    if (req.query['session_timeout'])
        doc.sessions.session_timeout = req.query['session_timeout'];
    else
        doc.sessions.session_timeout = 300;
