...
exports.topbar = function(head, req) {
    ...
    if (selectedThemeDoc) {
        var template = handlebars.compile(selectedThemeDoc.html);
...
exports.topbar_css = function(head, req) {
    ...
    while ((row = getRow())) {
        if (row.key[0] === 0) settingsDoc = row.doc;
        if (row.key[0] === 1) selectedThemeDoc = row.doc;
    }
    var settings = _.defaults(settingsDoc, dashboard_settings.defaultSettings);
    ...
    if (selectedThemeDoc) {
        var template = handlebars.compile(selectedThemeDoc.css);
    }
