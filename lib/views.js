...
exports.get_syncs = dashboard_views.get_syncs;
...
exports.user_data = {
    map: function(doc) {
        if (doc.type !== 'install') return;
        if (doc.kanso && doc.kanso.config && doc.kanso.config.user_data) {
            emit(null, {
                title: doc.dashboard_title,
                db : doc.installed.db,
                ddoc: doc.doc_id,
                user_data: doc.kanso.config.user_data
            });
        }
    }
}