    ...
    function GardenerStatus(dashboard_url, ddoc_path, elem_id) {
        ...
        pico_couch_ddoc.view(dashboard_url,  'gardener',  getView(), function(err){
            ...
            new RactiveCouch.View(dashboard_url, 'gardener/gardener', {
                el : elem_id,
                template: '{{#rows.length}}<h4>Modules</h4>{{/rows.length}}{{#rows}}<div><h6>{{key[1]}}</h6><div class="{{value.complete}}">{{value.msg}}</div><div class="progress" ><div class="bar" style="width: {{value.percent}}%; "></div></div></div>{{/rows}}',
                include_docs: true,
                view_options: {
                    startkey: [ddoc_path],
                    endkey: [ddoc_path, {}, {}],
    ...
    function map_change(change) {
        return {
            ...
            value: {
                ...
                complete: change.doc.percent === 100 ? 'complete' : 'working',
                msg: change.doc.msg
            }
        };
    }
